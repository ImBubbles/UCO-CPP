...
float InputWrapper::getFloat(const std::string &prompt) {
    ...
    while(true) {
        ...
        try {
            ...
            std::string message = "Invalid number entered, please enter a number";
            print(message);
        }
    }
    return result;
...
long long InputWrapper::getLongLong(const std::string &prompt) {
    double result;
    while(true) {
        std::string raw = ask(prompt);
        try {
            result = std::stoll(raw);
            break;
        }
        catch (const std::exception& e) {
            std::string message = "Invalid number entered, please enter an integer";
...
bool InputWrapper::getBoolean(const std::string &prompt) {
    std::string answer;
    bool result = false;
    while(true) {
        ...
        if(answer=="Y" || answer == "YES" || answer == "TRUE" || answer == "1") {
            result = true;
            break;
        } else if(answer=="N" || answer == "NO" || answer == "FALSE" || answer == "0") {
            result = false;
            break;
        }
    }
